...
const Cart = () => {
  const { state, removeItem, clearCart, increment, decrement, setTotalAmount } =
    GlobalShop();
      ...
      <Route exact path={path}>
        <div className="cart-container">
          <div className="cart-item-container">
            <p
              style={{ color: "#f50057", fontWeight: "bold", padding: "10px" }}
            >
              MY CART:{" "}
              <span className="theme-text">
                {state.cart.map((i) => i.quantity).reduce((a, b) => a + b, 0)}{" "}
                Item{state.cart.length < 1 ? "" : "s"} in cart
              </span>
            </p>
            {state.cart.length < 1 ? (
              <div className="theme-text">
                No item in cart{" "}
                <Link className="bordered" to="/buy-item">
                  Start Buying
                </Link>
              </div>
            ) : (
                    ...
                    <Box display="flex" className={classes.mainCon}>
                      ...
                      <div className={classes.cartText}>
                        ...
                        <Box display="flex" className={classes.track}>
                          <span className="theme-text">Hoovaa</span>{" "}
                          <span
                            style={{ display: "flex", alignItems: "center" }}
                          >
                            Fast Track <RiEBike2Fill className="theme-text" />
                          </span>
                        </Box>
